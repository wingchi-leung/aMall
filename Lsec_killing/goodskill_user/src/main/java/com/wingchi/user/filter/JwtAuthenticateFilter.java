...
public class JwtAuthenticateFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest httpServletRequest,
        ...
        String token = httpServletRequest.getHeader("token");  // 获取jWT
        ...
        if(JwtUtil.isTokenExpired(token)){
            log.info("token is exipred!!!!");
            filterChain.doFilter(httpServletRequest,httpServletResponse);
            return ;
        }
        Claims claims = JwtUtil.getClaims(token) ;
        String userId = (String) claims.get("user_id");  //获取之前放进jwt的字段
        if(userId!=null  )  {
            ...
            if(user!=null && userId.equals(String.valueOf(user.getId()))  ){
                ...
                SecurityContextHolder.getContext().setAuthentication(authentication);  //认证通过,把信息存进SecurityContextHolder
            }
        }
        filterChain.doFilter(httpServletRequest,httpServletResponse);
    }
